...
static void init_qu8_rdsum_config(void) {
    ...
    if (hardware_config->use_arm_neon) {
      qu8_rdsum_config = (struct xnn_reduce_config) {
        .rd_ukernel = (xnn_rdsum_ukernel_fn) xnn_qu8_rdsum_ukernel_7p7x__neon_c32,
        .element_tile = 32,
      };
    } else {
      qu8_rdsum_config = (struct xnn_reduce_config) {
  ...
  #elif XNN_ARCH_ARM64
    qu8_rdsum_config = (struct xnn_reduce_config) {
      .rd_ukernel = (xnn_rdsum_ukernel_fn) xnn_qu8_rdsum_ukernel_7p7x__neon_c16,
      .element_tile = 16,
    };
  #elif XNN_ARCH_X86 || XNN_ARCH_X86_64
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    ...
    if (hardware_config->use_x86_sse4_1) {
      qu8_rdsum_config = (struct xnn_reduce_config) {
        .rd_ukernel = (xnn_rdsum_ukernel_fn) xnn_qu8_rdsum_ukernel_7p7x__sse41_c64,
        .element_tile = 64,
      };
    } else {
      qu8_rdsum_config = (struct xnn_reduce_config) {
...
static void init_qu8_rsum_config(void) {
    ...
    if (hardware_config->use_x86_sse4_1) {
      qu8_rsum_config = (struct xnn_reduce_config) {
        .ukernel = (xnn_reduce_ukernel_fn) xnn_qu8_rsum_ukernel__sse41_u64,
        .element_tile = 64,
      };
    } else {
      qu8_rsum_config = (struct xnn_reduce_config) {
  ...
  qu8_rsum_config.init.qs8_mean = xnn_init_qs8_mean_minmax_scalar_params;
}
